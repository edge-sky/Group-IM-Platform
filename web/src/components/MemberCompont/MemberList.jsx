...
const MemberList = ({open, currUid, conversationId, onOpenChange, chats}) => {
    ...
    const fetchData = async () => {
        if (conversationId) {
            ...
            api.get(`/${conversationId}/conversation/userInfo`)
                .then(response => {
                    setMembers(response);
                    memberPermission.current = response.some(member => member.uid === currUid && member.memberPermission);
    ...
    const canRemoveMember = () => {
        ...
        // 如果选中的是自己，任何人都可以移除自己
        if (selectedMember.uid === currUid) return true;

        // 如果选中的不是自己，需要有成员管理权限
        if (!memberPermission.current && !isCreator.current) return false;
...
// 处理移除
    const handleRemove = (removeUid) => {
        // 如果移除的是自己，不需要权限检查
        const isSelf = removeUid === currUid;

        if (!isSelf && !memberPermission.current && !isCreator.current) return;

        const confirmMessage = isSelf ? '确定要退出此会话吗？' : '确定要移除此成员吗？';

        if (window.confirm(confirmMessage)) {
            api.post(`/${conversationId}/conversation/removeUser`, {
                removeUid: removeUid
            }).then(() => {
                showToast(isSelf ? '已退出会话' : '成员已移除', 'success');
                if (isSelf) {
                    // 如果是自己退出，关闭弹窗
                    onOpenChange(false);
                } else {
                ...
                showToast(isSelf ? '退出失败' : '移除失败', 'error');
                                        ...
                                        <div className="flex items-center mb-6">
                                            ...
                                            <div>
                                                ...
                                                {selectedMember.uid === conversationCreator && (
                                                    <span
                                                        className="inline-block mt-2 text-sm bg-yellow-100 text-yellow-800 px-3 py-1 rounded-full">
                                                        会话创建者
                                                    </span>
                                                )}
                                            </div>
                                    ...
                                    {canRemoveMember() && (
                                        <div className="mt-auto">
                                            ...
                                            >
                                                ...
                                                {selectedMember.uid === currUid ? '退出会话' : '移除成员'}
                                            </Button>
                                        </div>
                                    )}
                                </>
